...
//  ascii-shots
//
//  Created by Andrew Brandt on 4/25/15.
...
- (void)viewDidAppear:(BOOL)animated {
    //self.coreImageView.frame = self.view.frame;
    NSLog(@"Frame height: %f Frame width: %f", self.view.frame.size.height, self.view.frame.size.width);
    self.coreImageView = [[ASCoreImageView alloc] initWithFrame:self.view.frame];
    [self.view addSubview:self.coreImageView];
    [self.coreImageView startCapture];
}
